...
const ChartjsPolygonal: React.FC<Props> = memo(({ timeUnit, allLabels, allDatasets }) => {
  // const data = useMemo(() => {
  //   const labels: string[] = []
      ...
      <div className='chartjsPolygonal__legend' style={{ height: 60 }}>
        {allDatasets.map(({ label, color }, index) => (
          <span className='chartjsPolygonal__legend__item' key={index}>
            <span className='chartjsPolygonal__legend__item__color' style={{ backgroundColor: color }}/>
            <span className='chartjsPolygonal__legend__item__label'>{label}</span>
          </span>
        ))}
      </div>
            ...
            callbacks: {
              ...
              label: (tooltipItem) => {
                ...
                if (data.datasets.length > 1) {
                  const targetDatasets = data.datasets.map((dataset, index) => ({
                    label: dataset.label,
                    datasetIndex: index,
                    data: dataset.data[tooltipItem.dataIndex],
                    order: dataset.order
                  })).filter(dataset => dataset.data === tooltipItem.raw).sort((a, b) => a.order - b.order)
                  console.log(targetDatasets)
                  if (targetDatasets.length > 1) {
                    if (targetDatasets[0].datasetIndex === tooltipItem.datasetIndex) {
          ...
          afterDraw: (chart: { tooltip?: any; scales?: any; ctx?: any }) => {
            if (chart.tooltip._active && chart.tooltip._active.length) {
              ...
              if (topY < bottomY) {
                ...
                ctx.lineWidth = 1;
                ctx.strokeStyle = chart.tooltip.labelColors[0]?.borderColor || 'transparent';
                ctx.stroke();
                ctx.restore();
              }
            }
